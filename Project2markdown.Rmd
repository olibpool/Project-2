...
testing1 <- read.csv("Ailsatestid.csv",header=TRUE)
```
...
Random forest fit (mtry=p/3)
```{r}
set.seed(0597)
index <- sample(nrow(train), size = floor(0.5*nrow(train)))


RF2 <- randomForest(isFraud ~. , data=train, subset=index, mtry=15, ntree=100, importance=TRUE)
...
testing <- testing %>% mutate(P_emaildomain = fct_collapse(P_emaildomain, "other"=c("other","scranton.edu")))
...
submit <- testing1[y]
submit$isFraud <- impute(submit$isFraud,mean)


write.csv(submit,"submitidRF.csv",row.names=FALSE)
